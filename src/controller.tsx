...
class controller{
	...
	game : game;
	view : any;
	game_interval : any | undefined;
	fight_next_render : number | undefined;
	fight_framerate : number;
		...
		if(game.game_state().name == "fighting"){
			if(this.fight_next_render == undefined){
				...
				while(this.fight_next_render < now){
					game.fight_tick(); 
					if(game.game_state().combat_instance == undefined){ // if fight is over, don't continue
						break;
					}
	...
	//	console.log([e.pageX, e.pageY]);
		...
		if(state.name == "town"){ // we're in a town, check where we clicked and if we clicked on something useful
			var town_coords = T.town_data[(game.game_state().town as town).name];
			var index = 0;
	...
	//	console.log(e.code);
		...
		if(interface_state == "game"){
			...
			if(state2.name == "fighting"){
			...
			else if(state2.name == "fight end"){
				let state = state2 as fs.stack_frame_fight_end;
				if(e.code == "KeyA" && state.selected != 0){
					state.selected -= 1; 
					rerender();
				}
				if(e.code == "KeyS" && state.selected != state.items_dropped.length-1){
					state.selected += 1; 
					rerender();
				}
				if(e.code == "KeyQ" && state.chosen[state.selected] != undefined){
					state.chosen[state.selected]  = !state.chosen[state.selected] 
					rerender();
				}
				if(e.code == "Space"){
					game.finished_items();
			...
			} else if (state2.name == "dungeon"){ // pressed a button in dungeon		
				let state = state2 as fs.stack_frame_dungeon;			
				switch(e.code){
					case "KeyW":
					case "ArrowUp":
				...
				rerender();
			} else if (state2.name == "town" || state2.name == "dungeon"){
				if (e.code == "KeyQ"){
			...
			else if (state2.name == "dungeon end"){
				game.game_stack.pop();
				rerender();
			}
			else if (state2.name == "quest giver"){
				if(e.code == "Space"){
