...
import I_Dungeon from "./I_Dungeon";
...
class I_Combat{
	player : player
	fighting_monsters : monster[]; 
	dungeon : undefined  | I_Dungeon;
	fought_monsters : monster[];
	monster_list : monster[] // not mutated
	items_dropped : item[];
	currency_dropped : currency_obj;
	currently_queued_attack : number;
	currently_attacking_monster : number;
	...
	player_current_cd : number; // in ticks.
	player_effects	: effect_collection
	fight_ended : boolean;
	cooldowns : number[];
	constructor(player, monster_list : monster[], dungeon : undefined | I_Dungeon){ // does not mutate these
	// dungeon can be undefined
		...
		for(var i=0; i<monster_list.length; i++){
			this.fighting_monsters.push(_.clone(monster_list[i]));
	 ...
	 calculate_stats() : [number, number, number, number]{ // returns the player's stats
	 ...
	 fight_begin(queued_attack : number, target : number) : void{
	 ...
	 add_monster(monster : monster) : void{
		this.fighting_monsters.push(monster)
		
	}
	 has_other_effect(unit : "player" | monster, name : string) : boolean{
	...
	has_item(item_name : string) : boolean{
		for(var i=0; i<this.player.items.length ;i++){
			...
			else if((this.player.items[i] as item).name == item_name){
				return true;
			}
		}
		return false;
	...
	has_item_with_tag(item_name : string) : boolean{
		for(var i=0; i<this.player.items.length ;i++){
			if(this.player.items[i] == undefined){
				continue;
			}
			else if((this.player.items[i] as item).has_flag(item_name)){
				return true;
			}
		}
		return false;
	...
	get_monster_by_name(monster_name : string) : monster | undefined{
	...
	does_attack_succeed(attack_index : number, target_monster : monster ) : boolean{ 
			var attack = this.player.skills[attack_index]
			if(attack == undefined){ 
				return false;
			}
			attack = attack as player_skill;
	 ...
	 // given a number and list of effects, multiply that number by each effect's strength
	multiply_through_effects(number : number, effects : effect[]) : number{
	 ...
	 calculate_damage_p2m(attack_index : number, target_monster : monster ) : number{ 
			var attack = this.player.skills[attack_index]
			if(attack == undefined){
				return 0;
			}
			...
			// non-default effects
				for(var effect of this.player_effects.other){
					// (player's) effects, well , effect starts here.
				}
				
				// same for monster
				for(var effect of target_monster.effects.other){	
					// (monster's) effects, well , effect starts here.
				}
			return Math.floor(tentative_damage);
	}
	calculate_mana_cost(attack_index: number, target_monster : monster): number{
				var attack = this.player.skills[attack_index]
				if(attack == undefined){
					return 0;
				}
	...
	// "global" cooldown
	 calculate_delay_p2m(attack_index: number, target_monster : monster): number{ 
				var attack = this.player.skills[attack_index]
				if(attack == undefined){
					return 0;
				}
	...
	calculate_cd_p2m(attack_index: number, target_monster : monster): number{ 
			var attack = this.player.skills[attack_index]
			if(attack == undefined){
					return 0;
				}
	 ...
	 apply_effects_p2m(attack_index : number, target_monster : monster) : void{ 
		var attack = this.player.skills[attack_index]
		if(attack == undefined){
			return ;
		}
	...
	//target_index is either "player", or the index of a monster
	 ...
	 calculate_damage_m2p(attack : monster_skill, attacking_monster : monster) : number{
	 ...
	 calculate_damage_m2m(attack : monster_skill, attacking_monster : monster, target_monster_index : number) : number{
			var is_damage = (attack.damage > 0);
	 ...
	 calculate_delay_m2p(attack : monster_skill, attacking_monster : monster) : number{
		var tentative_delay = attack.delay; 				
						
		tentative_delay = this.multiply_through_effects(tentative_delay, attacking_monster.effects.speed)	
			
		// // non-default effects 
				
				for(var effect of attacking_monster.effects.other){
					// (monster's) effects, well , effect starts here.
				}
	 ...
	 apply_effects_m2p(attack : monster_skill, attacking_monster : monster) : void{
		//self 
		for(var effect of attack.self_effects){
	 ...
	 apply_effects_m2m(attack : monster_skill, attacking_monster : monster, target_monster_index : number) : void{
		var target_monster = this.fighting_monsters[target_monster_index];
		if(target_monster == undefined){
	...
	add_effect(current_effects : effect_collection, new_effect : effect) : void{
		...
		switch(new_effect.name){
			...
			case "speed":
			case "mana":
				current_effects[new_effect.name].push(_.clone(new_effect));
			break;
			default:
				current_effects["other"].push(_.clone(new_effect)); 
			
		}
	}
	
	//returns list of expired effectss
	decrement_effect_duration(effects : effect_collection) : effect[]{
	//	console.log(effects.name)
	//	console.log(JSON.stringify(effects));
		var keys = Object.keys(effects);
		var expired_effects : effect[] = [];
		for(var x of keys){
			if(x == "name"){
				continue;
			}
	...
	//when multiple effects expire, these will be called in order of decreasing index;
	 player_effect_expire(effect : effect) : void{
		if(effect.name == "delay damage"){
			this.player_hp -= Math.floor(effect.strength);		 	
		}
	}
	 monster_effect_expire(monster : monster, effect : effect) : void{
		if(effect.name == "delay damage"){
	...
	//when multiple monsters die, this will be called in order of decreasing index.
	 monster_death(monster_index : number) : void{
	...
	// monster attacks-------------------
			...
			var monster_attack : any = compute_attack_pattern(current_monster, this, z);
			...
			if(current_monster.current_delay == 0 && monster_attack != null){
				...
				if(attack_succeeds){// monster attack succeeds.
					...
					// delay done the same way as monster to player.
						var damage = this.calculate_damage_m2m(monster_attack, current_monster, monster_attack_target);
