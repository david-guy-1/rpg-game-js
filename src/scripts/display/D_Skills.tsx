...
import DC_skill_desc from "./DC_skill_desc";
import {game_context} from "../../AppContext";
import player_skill from "../classes/player_skill"

class D_Skills extends React.Component {
	props : {skills : (player_skill | undefined)[], equip :  (player_skill | undefined)[], selected_skill : number,selected_equip : number }
	constructor(props : {skills :  (player_skill | undefined)[], equip :  (player_skill | undefined)[], selected_skill : number,selected_equip : number }){
		super(props);
		this.props = props; 
	}
	render(){
				...
				function(){
					...
					for(var i=0; i<c.skill_table_rows; i++){
						for(var j=0; j<c.skill_table_cols;j++){
							...
							var skill = skills[d];
							//console.log(d)
							components.push(<div style={{"position":"absolute", "top":c.skill_name_height * i,"left":c.skill_name_width * j,"width":c.skill_name_width,"height":c.skill_name_height,"backgroundColor" : selected_skill == d ? "cyan" : "white"}} onClick={function(this : number){view.selected_skill=this;controller.rerender()}.bind(d)}>
		...
		{// render boxes.
				function(){
					...
					for(var i=0; i<10; i++){
							...
							components.push(<div style={{"position":"absolute", "top":c.skill_name_height *Math.floor(i/5),"left":c.skill_name_width * (i%5), "width":c.skill_name_width,"height":c.skill_name_height,"backgroundColor" : selected_equip == i? "cyan" : "white"}}
							
							onClick = {function(this : string){view.move_equipped_skill(this);controller.rerender()}.bind(skill_labels[i])}
		...
		<button style={{"position":"absolute", "top":c.skills_equip_top_left[1],"left":c.skills_equip_top_left[0], "width":c.skills_equip_width, "height":c.skills_equip_height,"backgroundColor":"lightgreen"}} onClick={function(){game.equip_skill( view.selected_skill, view.selected_equip_skill); controller.rerender();}}> <h2 style={{"textAlign":"center"}} > Equip skill</h2></button>
		
		<button style={{"position":"absolute", "top":c.skills_back_top_left[1],"left":c.skills_back_top_left[0], "width":c.skills_back_width, "height":c.skills_back_height,"backgroundColor":"lightblue"}} onClick={function(){view.leave_skills(); controller.rerender();}}> <h2 style={{"textAlign":"center"}} > Go back</h2></button>
