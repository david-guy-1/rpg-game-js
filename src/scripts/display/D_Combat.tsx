...
class D_Combat extends React.Component {
	props: {"combat":I_Combat};
	zero_last_tick: boolean;
	constructor(props : {"combat":I_Combat}){
		super(props);
	    ...
	    return (
		  ...
		  { 
			...
			for(var i=0; i<combat.fighting_monsters.length;i++){
				...
				onClick = {function(this : number){((game as unknown ) as game).set_currently_queued_attack(this)}.bind(i)}
				><DC_monster monster={combat.fighting_monsters[i]} key={i}  id={i} target = {combat.currently_attacking_monster == i} /> </div>);

			}
			return components;
			}
		  }</game_context.Consumer>
		  ...
		  { 
				...
				for(var i=0; i<10; i++){
					if(combat.player.skills[i] != undefined){
						// not on cooldown
							components.push(<div onClick = {function(this : number){((game as unknown ) as game).set_currently_attacking_monster(this)}.bind(i)} style={{ "position":"absolute", "top" : c.skill_top_left[1] +(i < 5 ? 0 : true_height), "left" : c.skill_top_left[0]  + (true_width)*(i%5)}}><DC_skill key={i} id={i} skill={combat.player.skills[i] as player_skill}  selected={combat.currently_queued_attack == i} cooldown={combat.cooldowns[i]}/></div>);
	...
	{ /* health bars */ }
		...
		{"position":"absolute","top":c.player_health_bar_top_left[1]+"px","left":c.player_health_bar_top_left[0],"width":c.player_health_bar_width + "px", "height":c.player_health_bar_height + "px", "backgroundColor":"grey",}

}> <div id="player_health_bar_inner" style={
		{"width": (combat.player_hp*100 / combat.player_max_hp) + "%", "height":c.player_health_bar_height  + "px", "backgroundColor":"green",} 
}> </div></div>
	...
	<table id="player_stats" style = {{
		...
		"backgroundColor":zero_twice? "cyan" : "white",
}}> <tbody>
	...
	<tr style={{"height":c.player_stats_height + "px",}}><td colSpan={5} id="player_effects_2"> 
		...
		function(){
			if(combat.current_ticks == 0){
				...
				var component : any[] = [];
				var effects : Set<effect> = U.union_lst(combat.player_effects) as Set<effect>;
