...
describe("load Dungeon", function() {
	...
	it("Should be able to select all items", function(){
		...
		return new Promise(async function(resolve, reject) {
				...
				Promise.all([RElem.getText(), RElem.getTagName()]).then((x) => console.log(x));
...
describe("end", function() {
	it("Should be able to finish", function(){
	return new Promise((x, y) => x(1));
	/**
	...
	after(function() {
		...
		browser.executeScript("return confirm('All tests passed, close?')").then((x)=> function(){
			if(x){
				browser.quit()
			}
		})
