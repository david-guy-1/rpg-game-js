...
export function convert(m : number) : number{ // converts m to unsigned
	if(m < 0){
		m = (2**32) + m;
	}
	return m; 
...
export function or(a: number,b: number): number{ // bitwise or, as 32 bit numbers
	return convert( a | b);
}
export function and(a: number,b: number): number{ // bitwise or, as 32 bit numbers
	return convert( a & b);
}

export function xor(a: number,b: number): number{ // bitwise or, as 32 bit numbers
	return convert( a ^ b);
}
export function not(a: number): number{
	return convert(~a);
}
export function rotate(number: number ,amount: number): number{
	var m =or(convert(number >>> amount ), convert(number  << (32-amount)))
	return m;
}

export function to_32_bit(a: number,b: number,c: number,d: number): number{ // 4 numbers 0-255 to a single 32-bit number
...
export function add_zeroes(x : number[]) : number[]{ // add zeroes until the result is length 8
	if(x.length >= 8){
		return x;
	}
	var m = [0];
...
var rounds = 64;
export function sha256_core(m : number[]) : number[]{ // m is a list of numbers ,each 8 bits. note that each letter is 8 bits.
...
export function string_to_bytes(m : string) : number[]{
	m = m.toString();
	var s  : number[] = [];
...
export function get_number(seed : string) : number{
...
export function choice(list : any,seed : string) : any{
...
export  function minus(a : Set<any>, b : Set<any>) : Set<any>{
	var Y = new Set();
	for(var i of  a){
		if(!b.has(i)){
			Y.add(i)
...
function array_equal(a : any[], b : any[]) : boolean {
  if (a === b) return true;
...
export function fillArray(value : any, len : number) : any[]	{
  if (len == 0) return [];
...
export function addObject(obj1 : object, obj2 : object ): void{
	console.log("adding")
...
export function check_type(x, y, error = ""){
...
export function union_lst(obj : object) : any{
	var array : any[] = [];
	for (var key in obj){
		var value = obj[key];
		if(Array.isArray(value)){
			array = array.concat(value as any[]);
		}
	}
	return array;
}
...
export function bfs(rows, cols, walls, start_x, start_y){
	var unsearched : Set<string>= new Set([start_x + " " + start_y])
	var reachable : Set<string> = new Set([start_x + " " + start_y]);
	while(unsearched.size != 0){

		// iterate over unsearched 
		var new_unsearched :  Set<string>= new Set();
