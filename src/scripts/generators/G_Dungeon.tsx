...
import monster_generator from "./G_Monster";
...
function dungeon_generator(name : string, difficulty : number , seed : string, wall_density : number , enemy_density : number , key_limit : number | undefined  , rows : number , cols : number ) : dungeon{
	U.assert(U.check_type(arguments, ["string", "number", "string", "number", "number", "number", "number", "number"], "dungeon generator type error"))
	var walls :Set<any> = new Set();
	var occupied : Set<string> = new Set(["0 0"]); // occupied places, as strings
	var entities : [number, number,dungeon_entity][] = [];
	var locks : any[] = [];
	var keys : any[] = []; 
	...
	while( !reachable.has((cols-1) + " " + (rows-1) )){
		...
		if(U.minus(reachable, occupied).size != 0 && key_limit != undefined && occupied.size < (key_limit as number) + 1){
